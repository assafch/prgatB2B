...
export function queryCatalog(
  ...
  const discount = getSettingBool('discount_pricing_enabled', false) ? resolveDiscount(custname) : null;
  ...
  const items: CatalogItem[] = rows.map((r) => ({
    ...
    price: usePersonal ? r.personal_price ?? r.list_price : effectivePrice(r.list_price, discount ? discount.forPart(r.partname) : null),
...
export function getProduct(partname: string, custname: string | null): CatalogItem | null {
  ...
  return {
    ...
    price: usePersonal ? row.personal_price ?? row.list_price : effectivePrice(row.list_price, discount ? discount.forPart(row.partname) : null),
